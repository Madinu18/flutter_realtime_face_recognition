...
List<List<double>> preprocessImage(String imagePath,
    {List<int> targetSize = const [112, 112]}) {
  try {
...
double calculateCosineSimilarity(
    List<double> embeddedA, List<double> embeddedB) {
  if (embeddedA.length != embeddedB.length) return 0;

  double dotProduct = 0.0;
  double normA = 0.0;
  double normB = 0.0;

  for (int i = 0; i < embeddedA.length; i++) {
    dotProduct += embeddedA[i] * embeddedB[i];
    normA += embeddedA[i] * embeddedA[i];
    normB += embeddedB[i] * embeddedB[i];
  }
  ...
  double percentage = (dotProduct / (normA * normB)) * 100;
...
Future<List<double>> getEmbeddedVector(XFile img1) async {
  try {
    final List<List<double>> processedInput =
        await compute(preprocessImage, img1.path);
